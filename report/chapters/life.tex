...
% TODO (CEO confirmation): exact sprint cadence and meeting names
...
\subsection{Norms}
A few unwritten communication norms shaped how the team worked:

\begin{itemize}[leftmargin=*]
    \item \textbf{Default to public channels.} Questions go into
    team channels rather than direct messages so the answers benefit
    the rest of the team and remain searchable.
    \item \textbf{Self-contained messages.} Provide enough context in
    a single message that the reader does not need a preamble of
    back-and-forth to act.
    \item \textbf{Show your working.} When asking for help, share
    what you have already tried and what you observed --- not just
    ``it does not work''.
    \item \textbf{Timezone empathy.} Avoid expecting immediate
    responses from teammates working in a different time zone;
    structure questions so that they can be answered overnight and
    come back actionable in the morning.
\end{itemize}
...
pointed at the project's \texttt{README.md} and \texttt{CLAUDE.md}
files, walked through the architecture by my supervisor, and
...
\section{Cross-Time-Zone Collaboration}
Working from Bangladesh on a team partly based in San Francisco
required intentional handoffs. The roughly twelve-hour offset between
the two time zones meant there were only a few overlap hours per
day, and they were precious. I learned to:

\begin{itemize}[leftmargin=*]
    \item Leave detailed end-of-day status updates so the United
    States team could pick up where I left off.
    \item Compose questions that did not require immediate replies
    --- so the United States team could answer them overnight and I
    would have actionable information the next morning.
    \item Schedule the small number of synchronous meetings during
    the evening overlap window without disrupting either side's
    focus time.
\end{itemize}

By the end of the internship, this cross-time-zone discipline felt
entirely natural. It is one of the most concretely transferable
skills the experience gave me.

\section{Reflection on Remote Work as an Intern}
...
experience challenged that assumption. Working remotely on a globally
distributed team taught me a different but equally valuable set of
skills: written clarity, asynchronous coordination, self-direction,
operational ownership of my own development environment, and the
empathy required to collaborate across continents and time zones.
These are precisely the skills that a senior software engineering
career increasingly demands.

What I missed --- the casual hallway conversations, the in-person
whiteboard sessions --- I missed honestly. But what I gained was
substantial: the experience of contributing to a serious product
alongside experienced engineers in Silicon Valley, with the agency
and trust that comes from being treated as a contributing member of
