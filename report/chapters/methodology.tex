...
The descriptions in this chapter are drawn from the project's internal
\texttt{README.md} and \texttt{CLAUDE.md} files (the latter being the
team's onboarding-and-conventions document for AI-assisted coding
environments) and from my own day-to-day experience working inside the
process for five and a half months.

\section{Development Methodology}
% TODO (CEO confirmation): formal methodology name, sprint cadence
...
% TODO (CEO confirmation): exact number of approvals required
...
\subsection{Metadata Generation Pipeline}
A defining characteristic of the connectivity framework is that the
runtime, the auth-configuration UI, and the metadata catalogue are
all derived from the connector source code by a deterministic
three-step pipeline:

\begin{enumerate}[leftmargin=*]
    \item \texttt{generate\_openapi\_spec.py} introspects the
    \texttt{@openapi\_endpoint}-decorated methods and produces an
    \texttt{openapi\_spec.json} for the connector.
    \item \texttt{generate\_auth\_config.py} introspects the
    \texttt{ConnParams} Pydantic schema (and the
    \texttt{json\_schema\_extra} UI metadata on each field) and
    produces an \texttt{auth\_config.json} that drives the auth-form
    UI.
    \item \texttt{populate\_connector\_db.py} pushes the resulting
    metadata into the platform's PostgreSQL store, after which the
    connector is immediately discoverable in the FlowGenX UI.
\end{enumerate}

This pipeline is run for every new connector and re-run whenever
schemas, examples, or methods change. It is the bridge that turns
``code on disk'' into ``connector visible in the product''.

\subsection{Deployment to Development, Staging, and Production}
% TODO (CEO confirmation): exact environment names + cadence
...
\subsection{Hotfixes}
% TODO (CEO confirmation): formal hotfix process
...
turnaround. During my internship I contributed to several such
hotfixes, including the OAuth2 access-token refresh fix for
QuickBooks, the SQL Server retry logic, the Elasticsearch Windows
encoding fix, the Customer.io test configuration fix, and the
Google Chat malformed-request-body fix.

\section{Development Tools}
A summary of the tools used across the lifecycle:

\subsection{Version Control}
\textbf{Git}, hosted on \textbf{GitHub}. Work happens on isolated
feature branches; \texttt{dev} is the integration branch and the
source for environment promotions. Branch names are scoped per
connector or feature (for example, \texttt{wordpress-connector},
\texttt{Harness\_connector}, \texttt{connector/aws\_textract}).

\subsection{Project Management}
\textbf{ClickUp}. Each connector lives as a card with assignee,
status, links to its branch and pull request, and the four-stage
testing checklist (Implementation / API / MCP Playground / Workflow).

\subsection{Code Review}
\textbf{GitHub Pull Requests}. All review feedback happens inline in
the PR. Approvals are required before merging.

\subsection{Code Quality and Formatting}
\begin{itemize}[leftmargin=*]
    \item \textbf{Black}, with a 79-character line length, targeting
    Python 3.11+.
    \item \textbf{isort}, configured with the \emph{black} profile.
    \item \textbf{Ruff}, with the rule sets
    \texttt{E,W,F,I,C,B,UP,N,YTT,S} selected and
    \texttt{E501,S101,D100--D104} ignored.
    \item \textbf{Pre-commit hooks} that enforce all of the above
    automatically.
\end{itemize}

\subsection{Package Management}
\textbf{uv} --- a fast Python package manager that replaces
\texttt{pip} and \texttt{pip-tools} for both installation and
dependency resolution. Every developer environment is created by
\texttt{uv pip install -e ".[dev]"}.

\subsection{Containerisation and Orchestration}
\textbf{Docker} for containerisation, \textbf{Kubernetes} for
orchestration, \textbf{Helm} for Kubernetes packaging, and
\textbf{Terraform} for infrastructure as code.

\subsection{Communication and Collaboration}
\textbf{Google Chat} for asynchronous messaging and \textbf{Google
Meet} for synchronous discussions. Microsoft Teams was used
previously and was migrated away from during my tenure.
...
These conventions matter at scale: with more than two hundred
connectors live in the runtime, uniform error and log shapes are what
let the operations side of the company stay sane.
